 ...
 */
fun formatAuthorName(name: String): String {

    val nameParts = name.split(" ")
    if (nameParts.size == 1) return name.toUpperCase()
    if (nameParts.size == 2) return twoPartName(nameParts)
    if (nameParts.size > 2) return moreThanTwoPartName(nameParts)

    return ""
}

fun moreThanTwoPartName(nameParts: List<String>): String {
    val nameBuilder = StringBuilder()
    var i  = nameParts.size

    if (lastNameException.contains(nameParts.last().toUpperCase())
    && !separators.contains(nameParts[nameParts.size - 2])){
        nameBuilder.append(nameParts[nameParts.size - 2].toUpperCase()
                + " " + nameParts.last().toUpperCase())
        i -= 3
    } else {
        nameBuilder.append(nameParts.last().toUpperCase())
        i -= 2
    }
    ...
    for (it in 0..i) {
        if (separators.contains(nameParts[it].toLowerCase()) ) {
            nameBuilder.append(" " + nameParts[it].toLowerCase())
        }else{
            nameBuilder.append(" " + nameParts[it].capitalize())
        }
    }
...
fun twoPartName(nameParts: List<String>) = nameParts[1].toUpperCase() + ", " + nameParts[0].capitalize()

private val lastNameException = arrayOf("FILHO", "FILHA", "NETO", "NETA", "SOBRINHO", "SOBRINHA", "JUNIOR")
